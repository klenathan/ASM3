...
class CommunityRepository(PostgresRepositoryBase):
    ...
    def get_society(self, society_id: str) -> Society | None:
        with self._session() as session:
            row = session.scalars(select(Societies).where(Societies.society_id == society_id)).first()
            return Society.model_validate(_as_dict(row)) if row else None

    def get_society_by_slug(self, slug: str) -> Society | None:
        with self._session() as session:
            row = session.scalars(select(Societies).where(Societies.slug_lower == slug.lower())).first()
            return Society.model_validate(_as_dict(row)) if row else None

    def update_society(self, society_id: str, **changes: object) -> None:
        with self._session.begin() as session:
            self._update(session, Societies, Societies.society_id == society_id, changes)
    ...
    def reserve_slug(self, slug: str, society_id: str) -> bool:
        with self._session.begin() as session:
            return self._try_insert(
                session, SlugReservations, {"slug_lower": slug.lower(), "society_id": society_id}
            )
    ...
    def list_members(self, society_id: str) -> list[SocietyMembership]:
        with self._session() as session:
            rows = session.scalars(
                select(SocietyMemberships).where(SocietyMemberships.society_id == society_id)
            ).all()
    ...
    def list_joined(self, user_id: str) -> list[SocietyMembership]:
        with self._session() as session:
            rows = session.scalars(
                select(SocietyMemberships).where(SocietyMemberships.user_id == user_id)
            ).all()
            return [SocietyMembership.model_validate(_as_dict(row)) for row in rows]
    ...
    def get_post(self, post_id: str) -> Post | None:
        with self._session() as session:
            row = session.scalars(select(Posts).where(Posts.post_id == post_id)).first()
    ...
    def create_comment(self, comment: Comment) -> None:
        with self._session.begin() as session:
            self._upsert(session, Comments, _dump(comment), [Comments.comment_id])

    def get_comment(self, comment_id: str) -> Comment | None:
        with self._session() as session:
            row = session.scalars(select(Comments).where(Comments.comment_id == comment_id)).first()
            return Comment.model_validate(_as_dict(row)) if row else None

    def update_comment(self, comment_id: str, **changes: object) -> None:
        with self._session.begin() as session:
            self._update(session, Comments, Comments.comment_id == comment_id, changes)

    def list_comments(self, post_id: str) -> list[Comment]:
        with self._session() as session:
            rows = session.scalars(
                select(Comments).where(Comments.post_id == post_id).order_by(Comments.path.asc())
            ).all()
    ...
    def increment_counter(self, content_id: str, field: str, delta: int) -> None:
        ...
        with self._session.begin() as session:
            result = session.execute(
                update(Posts).where(Posts.post_id == content_id).values({field: getattr(Posts, field) + delta})
            )
    ...
    def delete_vote(self, content_id: str, user_id: str) -> None:
        with self._session.begin() as session:
            session.execute(delete(Votes).where(Votes.content_id == content_id, Votes.user_id == user_id))
    ...
    def create_report(self, report: Report) -> bool:
        with self._session.begin() as session:
            stmt = (
                ...
                .values(**_dump(report))
                .on_conflict_do_nothing(index_elements=[Reports.content_id, Reports.reporter_user_id])
    ...
    def list_open_reports(self, state: str = "OPEN") -> list[Report]:
        with self._session() as session:
            rows = session.scalars(
                select(Reports).where(Reports.state == state).order_by(Reports.created_at.asc())
            ).all()
            return [Report.model_validate(_as_dict(row)) for row in rows]

    def resolve_report(self, report_id: str, state: str) -> None:
        with self._session.begin() as session:
            session.execute(update(Reports).where(Reports.report_id == report_id).values(state=state, updated_at=utc_now()))

    def create_notification(self, notification: Notification) -> None:
        with self._session.begin() as session:
            self._upsert(session, Notifications, _dump(notification), [Notifications.notification_id])
    ...
    # ----------------------------------------------------------------- feeds
    def add_feed_entry(self, feed_type: str, feed_id: str, content_id: str, published_at: str) -> None:
        with self._session.begin() as session:
            session.execute(
    ...
    def list_joined_societies(self, user_id: str) -> list[str]:
        with self._session() as session:
            rows = session.execute(
                select(SocietyMemberships.society_id).where(SocietyMemberships.user_id == user_id)
            ).all()
            return [str(row[0]) for row in rows]
    ...
    def list_following(self, user_id: str) -> list[str]:
        with self._session() as session:
            rows = session.execute(
                select(Follows.target_user_id).where(Follows.follower_user_id == user_id)
            ).all()
            return [str(row[0]) for row in rows]
