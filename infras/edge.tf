...
resource "aws_cloudfront_origin_access_control" "s3" {
  name                              = "${local.name}-s3"
  description                       = "Private S3 access for web and media"
...
resource "aws_cloudfront_function" "spa_rewrite" {
  name    = "${replace(local.name, "-", "_")}_spa_rewrite"
  runtime = "cloudfront-js-2.0"
  comment = "Serve index.html for extensionless React Router paths"
  publish = true
  code    = <<-JS
...
resource "aws_cloudfront_distribution" "this" {
  ...
  depends_on = [aws_apigatewayv2_stage.backend]
}
