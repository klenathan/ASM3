...
export function ProfilePage() {
  ...
  const threadsQuery = useUserThreads(user?.userId ?? "", status === "authenticated")
  const commentsQuery = useUserComments(user?.userId ?? "", status === "authenticated")
  ...
  const toggleError =
    update.isError && !editOpen ? update.error?.message ?? "Could not update your profile." : null
  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 px-6 py-12 sm:py-16 lg:px-10">
      ...
      <section aria-labelledby="visibility-title" className="mt-8 border-t border-foreground/15 pt-6">
        <div className="flex items-start justify-between gap-6">
          <div className="min-w-0">
            <h2 id="visibility-title" className="font-heading text-xl font-semibold tracking-[0.01em] uppercase">
              Profile visibility
            </h2>
            <p className="mt-2 max-w-lg leading-7 text-muted-foreground">
              When on, other students can visit your profile and read your threads and
              comments. When off, only you can see your activity.
            </p>
            {toggleError !== null && (
